...
(function () {
  ...
  // -----------------------------------------
  // 3) Activités du mois (colonne G du planning annuel)
  // -----------------------------------------
  // Clé = numéro de mois (1=janvier … 12=décembre)
  const monthActivities = {
    1: ["10 janvier : galette des rois",
      "29 janvier : Randonnée pédestre - Talensac"
    ],
    2: ["15 février au 01 mars : vacances scolaires",
    ...
    8: ["02 au 09 août : Semaine fédérale à Château-Gontier",
      "29 août : Rando de l'Hermitage",
      "31 août au 04 septembre : Séjour Rando sacoche"],
    9: ["1er septembre : rentrée scolaire",
      "19 septembre : rando de Chartres-de-Bretagne",
      "24 septembre : Randonnée pédestre"
    ],
    10: ["04 octobre : rando VTT + marche – Vallée du Boël",
      "22 octobre : Randonnée pédestre"
    ],
    11: ["19 novembre : Randonnée pédestre",
      "20 novembre : AG Bicyclette Bruzoise"],
    12: ["05 décembre : renouvellement des licences",
      "17 décembre: Randonnée pédestre",
      "Bonne année 2027 ! 🎉"]
  };
  ...
  // Par défaut : ouvrir sur le mois courant (si on est bien sur l'année YEAR)
  ...
  function initIfUnlocked() {
    ...
    // Si on est sur l'année YEAR, on s'aligne sur le mois du jour
    if (todayKey) {
      const now = new Date();
    ...
    renderMonthSelector(monthSwitcherEl, () => {
      // Changement de mois : on garde juste le surlignage du jour courant (si visible)
      renderCalendar(monthTitleEl, calendarGridEl, detailsContent, currentYear, currentMonth, todayKey, null);
    });

    // Premier affichage : ouvrir automatiquement sur la date du jour (même si aucun circuit)
    renderCalendar(monthTitleEl, calendarGridEl, detailsContent, currentYear, currentMonth, todayKey, (didAutoOpenToday ? null : todayKey));
    didAutoOpenToday = true;
  }
  ...
  function renderCalendar(monthTitleEl, calendarGridEl, detailsContent, year, month, highlightDateKey, autoOpenDateKey) {
    ...
    // Mise en évidence du jour courant (si présent dans le mois affiché)
    if (highlightDateKey) {
      const todayCell = calendarGridEl.querySelector(`.day-cell[data-date="${highlightDateKey}"]`);
    ...
    // Toujours rafraîchir les activités du mois, quel que soit le chemin emprunté
  ...
  function renderMonthActivities(month) {
    const el = byId("month-activities-content");
    if (!el) return;
    const activities = monthActivities[month + 1]; // month is 0-indexed
    if (!activities || activities.length === 0) {
      el.innerHTML = "<p class=\"details-empty\">Aucune activité particulière ce mois-ci.</p>";
      return;
    }
    let html = "<ul class=\"activities-list\">";
    activities.forEach(a => {
      html += `<li>${a}</li>`; // les items peuvent contenir du HTML (liens, images)
    });
    html += "</ul>";
    el.innerHTML = html;
  }
